...
class DogStatsDClient {
  ...
  flush() {
    ...
    log.debug(
      `Flushing ${queue.length} metrics via ${this._httpOptions ? "HTTP" : "UDP"}`,
    );
  ...
  static generateClientConfig(config = {}) {
    ...
    if (config.tags) {
      Object.keys(config.tags)
        ...
        .forEach((key) => {
          ...
          const value = config.tags[key].replace(/[^a-z0-9_:./-]/gi, "_");
    ...
    } else if (!clientConfig.port && config.port) {
      clientConfig.metricsProxyUrl = new URL(
        format({
          protocol: "http:",
...
class CustomMetrics {
  ...
  gauge(stat, value, tags) {
    return this.dogstatsd.gauge(stat, value, CustomMetrics.tagTranslator(tags));
  }
